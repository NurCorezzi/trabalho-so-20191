...
void printJobInfo(NodeJob job){
	...
	char begin_date[50];
	char end_date[50];
	char pattern[50]="%d/%m/%y,%H:%M:%S";
	
	strftime (sub_date,50,pattern,localtime(&job.submission));
	strftime (begin_date,50,pattern,localtime(&job.begin));
	strftime (end_date,50,pattern,localtime(&job.end));
	...
	while(next != NULL){
		strftime (begin_date,50,pattern,localtime(&(next->begin)));
		strftime (end_date,50,pattern,localtime(&(next->end)));
	...
	if(argc != 2){
		...
		while(running){
			...
			if(next != NULL){
				...
				while(TRUE){
					MessageGerente info;
					while(msgrcv(qid, &info, sizeof(info)-sizeof(long int), ESCGER_MSG_TYPE, 0) == -1){
						if(errno == EINTR) continue;
						else{
							/*Deu erro cabuloso nao tem o que fazer*/
							endChild();
